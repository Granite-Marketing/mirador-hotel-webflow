 ...
 *
 * Walks the content tree at $CONTENT_ROOT (default `Content/`), uploads each
 * image to the Webflow asset library, dedupes via MD5 hash against existing
 * site assets, and writes a local manifest at assets/content-manifest.json
 * mapping every source path to a Webflow asset id + hosted URL.
 *
