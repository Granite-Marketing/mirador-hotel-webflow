...
## Baseline animation classes (`.slide-in`, `.fade-in`)

The `gsapBasicAnimations` module in `src/utils/gsapBasicAnimations.ts` runs
a `ScrollTrigger.batch` on two opt-in selectors:

- `.slide-in` — element rises into place with a slight Y-axis translation
- `.fade-in` — element fades from `opacity: 0` to `1`

Add either class in Webflow Designer to any element that should animate in
as it scrolls into view. The module batches reveals (so a row of cards
animates together, not sequentially per element), and the trigger is
keyed off `ScrollTrigger.batch`'s default start position (top 90% of
viewport). Initial state (`opacity: 0`, `y: <offset>`) is set on
DOMContentLoaded so the elements are hidden until the trigger fires —
do NOT rely on Webflow IX2 to set the initial state.

If your project already uses Webflow's native scroll-in interactions,
remove `.slide-in` / `.fade-in` from those elements to avoid double
animations.

---

## Accordion contract (`.accordion_header` / `.accordion_content`)

The `accordion` module in `src/utils/accordion.ts` drives accessible
accordion toggle behaviour with smooth max-height transitions and proper
...
```
.accordion_item
  .accordion_header        ← clickable trigger
  div                      ← contentWrapper (next sibling of header)
    .accordion_content     ← actual content; height of this element drives max-height
```

The module:

- Sets `aria-expanded` on each header (`false` by default, toggled on click)
- Generates matching `id` / `aria-controls` / `aria-labelledby` pairs per item
- Toggles an `is-active` class on the active header for CSS styling
- Closes any sibling accordion before opening a new one (single-open behaviour)
- Animates the wrapper's `max-height` from `0` to the inner content height

Styling: animate `max-height` and `opacity` on the contentWrapper via CSS
`transition`. Keep the inner `.accordion_content` at its natural height —
the module reads `offsetHeight` from it to compute the open height.
