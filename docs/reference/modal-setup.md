...
Use when: each CMS item has its own dialog (e.g. team members, projects,
testimonials). Two separate Collection Lists, both bound to the same CMS
collection: the **trigger list** lives in normal page flow; the **modal
list** lives at the page root (so each dialog's `position: fixed` resolves
to the viewport, not to a transformed ancestor). Slug-based ids on the
dialogs pair with the trigger cards' `data-modal-open` values, so each
card opens its corresponding dialog regardless of DOM relationship.
