 ...
 *
 * Walks the content tree at $CONTENT_ROOT (default `Content/`) category
 * folders, pairs each project folder with its docx body text, extracts fields,
 * cleans display names (strip YYYY- / NN- prefixes, title-case), and collates
 * the images in the folder.
 *
 * Per-project configuration: adapt CATEGORY_MAP to your project's category
 * folder naming.
 */
...
const CATEGORY_MAP = {
  '01-EXAMPLE-CATEGORY': { name: 'Example Category', slug: 'example-category' }
}

// Country normalisation — docx -> canonical display name.
// Extend with the countries that appear in your project's source content.
...
const FORCE_UPPER = new Set(['US', 'UAE', 'UK', 'II', 'III', 'IV'])
...
// Keyed by slugified folder-stripped name. Populate per-project as needed.
