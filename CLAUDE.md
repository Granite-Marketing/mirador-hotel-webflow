...
Editoria's modules use `import gsap from 'gsap'` and `import Swiper from 'swiper'` rather than treating them as CDN globals. So `gsap` and `swiper` are installed as npm deps and bundled into `dist/index.js` (~117KB minified). When you load the bundle in Webflow's footer, you do **not** also need to load `<script src="https://cdn.jsdelivr.net/npm/gsap"></script>` etc.

If you migrate any module to the template's CDN-globals pattern (removing the `import gsap` line and relying on the ambient global declared in `src/types/gsap.d.ts`), the bundle gets smaller and you'd load gsap/swiper via CDN tags instead.

---

Below: the conventions inherited from `webflow-mcp-starter`. Adapt as needed.
...
### Structure
- **Section wrapper:** `section_component-name`
- **Component elements:** `component-name_element` with underscore separator
- **Modifiers:** `is-` prefix (e.g., `is-active`, `is-animated`, `is-link`)
...
Update the Webflow footer tag to the new version to ship.
...
- Example: `card_component` + `is-large` for a larger variant of the same component
