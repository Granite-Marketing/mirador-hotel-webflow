...
## Hero-title reveal contract (text-animation sliders)

The `swiperSliders` module in `src/utils/swiperSliders.ts` runs a SplitText
mask reveal on hero titles each time the slide changes — but only when the
matching DOM contract is met. If your project needs the reveal, mark the
heading element inside each `.swiper-slide` with **all three** classes:

```html
<h1 class="heading-style-h1 hero_title is-slider">Slide heading</h1>
```

The module queries `.heading-style-h1.hero_title.is-slider` inside every
slide. Missing any of the three classes is a no-op — the slider still works,
the SplitText reveal just doesn't fire. This is intentional: the SplitText
path is opt-in by class contract, so projects that don't want the reveal
don't need to delete code.
