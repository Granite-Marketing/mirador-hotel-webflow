#!/usr/bin/env node
/**
 * Create News CMS items from a flat-text docx dump with dated entries separated
 * by blank lines and a "tag:" line per entry. Each entry's hero image is one of
 * the dated JPG/PNG files in the same folder (matched by month-year + keyword).
 *
 * Posts as drafts — does not publish.
 *
 * Per-project configuration: fill in the REPLACE_ME constants below from your
 * project's Webflow collection + option IDs. See `config.example.json` for the
 * canonical pattern; this script reads its IDs inline for clarity.
 */
...
const NEWS_COLLECTION = 'REPLACE_ME_NEWS_COLLECTION_ID'
const NEWS_CATEGORY_IDS = {
  'latest news': 'REPLACE_ME_CATEGORY_ID',
  'awards': 'REPLACE_ME_CATEGORY_ID'
}
